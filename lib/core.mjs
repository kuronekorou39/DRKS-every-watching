 ...
 *   finished: 終わった配信 [{ stream_id, start, end, title, game }]（アーカイブ等から正確な時刻が取れたもの）
 *   live:     配信中なら { stream_id, start, title, game }、いなければ null
 * - finished にある配信は開始・終了を確定（end_source: "vod"）
...
export function fromTwitch(videos, live) {
  ...
  for (const v of videos) {
    ...
    finished.push({ stream_id: v.stream_id, start, end: start + dur, title: v.title });
  }
  return {
    finished,
    live: live ? { stream_id: live.id, start: live.started_at, title: live.title, game: live.game_name } : null,
  };
}
...
export function fromYouTube(videos) {
  ...
  for (const v of videos) {
    ...
    const item = { stream_id: v.id, start: d.actualStartTime, title: v.snippet?.title };
...
/**
 * history.json を { channels: [{ name, icon, sources: [{ platform, key, channel, streams }] }] } の形にそろえる。
