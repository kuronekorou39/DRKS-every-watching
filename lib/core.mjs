...
/** "YYYY-MM-DD HH:MM" か "YYYY-MM-DD HH:MM:SS"（日本時間）→ UTC ミリ秒。形式が違えば NaN */
export function parseLocalDateTime(str) {
  const m = /^(\d{4}-\d{2}-\d{2})[ T](\d{1,2}):(\d{2})(?::(\d{2}))?$/.exec(str ?? '');
  return m ? Date.parse(`${m[1]}T${m[2].padStart(2, '0')}:${m[3]}:${m[4] ?? '00'}Z`) - TZ_OFFSET_MIN * MIN : NaN;
}

/** UTC ミリ秒 → "YYYY-MM-DD HH:MM:SS"（日本時間）。parseLocalDateTime の逆 */
export const localDateTimeString = (ms) =>
  new Date(ms + TZ_OFFSET_MIN * MIN).toISOString().slice(0, 19).replace('T', ' ');

/**
 * manual.json の1件を mergeHistory の finished に渡す形にそろえる。時刻がおかしければ例外。
 * 終わりは end（時刻）か duration（"3h8m33s" の形の長さ）のどちらかで書く
 */
export function fromManual(entry) {
  const start = parseLocalDateTime(entry.start);
  const end = entry.end != null ? parseLocalDateTime(entry.end) : start + parseDuration(entry.duration);
  if (!(start < end)) {
    throw new Error(`manual.json の時刻が正しくありません: ${entry.start} 〜 ${entry.end ?? entry.duration}`);
  }
  return {
...
/** 狭い場所に出すための短い書き方。9時間57分 → "9h57m"、5分 → "5m" */
