...
/**
 * これまでの履歴に、今回取得した VOD とライブ状態をマージして返す（引数は変更しない）。
 * - VOD がある配信は created_at + duration で開始・終了を確定（end_source: "vod"）
 * - ライブ中の配信は started_at と「今」で記録（end_source: "poll"）
 * - 前回ライブ中で今回いない配信は、最後に確認した時刻を終了として閉じる
 */
export function mergeHistory(streams, { videos = [], live = null, now = new Date() }) {
  ...
  for (const v of videos) {
    // ライブ中の配信の VOD は長さが伸び続けるので、終わるまで使わない
    if (!v.stream_id || v.stream_id === liveId) continue;
    const dur = parseDuration(v.duration);
    if (!dur) continue;
    const start = Date.parse(v.created_at);
    const cur = byId.get(v.stream_id);
    byId.set(v.stream_id, {
  ...
  if (live) {
    const cur = byId.get(live.id);
    byId.set(live.id, {
      stream_id: live.id,
      start: new Date(Date.parse(live.started_at)).toISOString(),
      end: now.toISOString(),
      end_source: 'poll',
      live: true,
      title: live.title ?? cur?.title ?? '',
      game: live.game_name || cur?.game || null,
    });
  }
...
/** history.json を { channels: [{ channel, streams }] } の形にそろえる（1チャンネル時代の形式も読む） */
export function normalizeHistory(history) {
  if (history?.channels) return history;
  if (history?.channel) return { channels: [{ channel: history.channel, streams: history.streams ?? [] }] };
  return { channels: [] };
}
